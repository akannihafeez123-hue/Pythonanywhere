#!/usr/bin/env python3
"""
app.py - Single-file Telegram bot (python-telegram-bot v20+), polling mode by default.

Features:
 - Solana RPC-only helpers (rpc_post)
 - Raydium / Orca pool parsers (best-effort) + generic reserve discovery
 - On-chain reserve-based price computation + CoinGecko fallback
 - Heuristic + optional RandomForest model (joblib) with SQLite persistence
 - Telegram commands: /start, /status, /analyze, /suggest, /train
 - Interactive approval flow with inline buttons
 - Polling mode (no webhook) if WEBHOOK_URL unset; use WEBHOOK_URL to enable webhook instead
"""
...
ADMIN_CHAT = os.getenv("ADMIN_CHAT")  # optional admin chat id (string)
...
async def cmd_analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if want_price:
        # Optional known pools mapping (populate for deterministic results)
        known_pools = {}  # {"mint":["poolAcc1","poolAcc2"]}
        price=None
...
async def cmd_suggest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # run discovery in separate thread to avoid blocking event loop
    def bg():
        cands = discover_candidates(seeds)
        if not cands:
            asyncio.run(context.bot.send_message(chat_id=chat_id, text="No candidates found."))
            return
        PENDING[chat_id] = {"candidates":cands, "i":0}
        asyncio.run(context.bot.send_message(chat_id=chat_id, text=format_candidate_msg(cands[0],0,len(cands)), reply_markup=InlineKeyboardMarkup([
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action=="approve":
        ...
        await context.bot.send_message(chat_id=chat, text=format_candidate_msg(candidates[state["i"]], state["i"], len(candidates)),
...
# Load model and optionally train on start (background)
load_model()
if TRAIN_ON_START:
    def bg_train():
        try:
...
application.create_task(heartbeat_task())

# --------------------
# Entrypoint: polling (default) or webhook (if WEBHOOK_URL set)
# --------------------
if __name__ == "__main__":
    print("Starting Pocket Option bot. Mode:", "webhook" if WEBHOOK_URL else "polling")
    if WEBHOOK_URL:
        # If you set WEBHOOK_URL, this runs PTB's webhook server (requires external reachable HTTPS URL)
        application.run_webhook(listen="0.0.0.0", port=PORT, webhook_url=WEBHOOK_URL)
    else:
        # Ensure no webhook is set for this token (so polling will work)
        try:
