...
async def cmd_suggest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def bg_discover():
        ...
        markup = InlineKeyboardMarkup([
            [InlineKeyboardButton("â Yes", callback_data=f"approve|0"), InlineKeyboardButton("â No", callback_data=f"reject|0")],
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if action == "approve"
    if ADMIN_ONLY and not is_admin(chat):
        await q.message.reply_text("âï¸ Youâre not authorized to publish tokens."  return
    ...
    if ADMIN_CHAT:
        ...
        markup = InlineKeyboardMarkup([
            [InlineKeyboardButton("â Yes", callback_data=f"approve|{state['i']}"), InlineKeyboardButton("â No", callback_data=f"reject|{state['i']}")],
